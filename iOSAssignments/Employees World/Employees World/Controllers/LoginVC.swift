...
//        }
    ...
    @IBAction func loginBtntapped(_ sender: Any) {
        if loginEmailTextField.text!.isEmpty || loginPswdTextField.text!.isEmpty {
            showToast(withMsg: "Email and Password should not be empty! Enter your login email and password")
        } else {
            callLoginAPI()
        }
    }
        ...
        NetwokManager.fetchData(urlRequest: RequestType<LoggedinUserData>.login(parameters)) { (result) in
            ...
            case .sucess(let loginData):
                ...
                if postLoginResponse.userStatus.statusCode == 1 {
                    ...
                    self.showToast(withMsg: postLoginResponse.userStatus.message ?? "")
                }
                break
        ...
        NetwokManager.fetchData(urlRequest: RequestType<ApiResponse>.forgetPassword(parameters)) { (result) in
            ...
            case .sucess(let response):
                ...
                if forgetPswdResponse.userStatus.statusCode == 1 {
                    ...
                    //self.viewTitleLabel.text = "Sign Up"
