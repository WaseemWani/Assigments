...
class LAAnnularLayer: CAShapeLayer {
    ...
    var nextCircleCenterColor: UIColor?
    ...
    var showFlag: Bool = true {
        didSet {
            if !showFlag {
                ...
                self.flagLayer.contents = LAAnnularLayer.flagCGImage
            }
        }
    }
    ...
    override init() {
        ...
        if LAAnnularLayer.flagCGImage == nil {
            var flagImage = UIImage(named: LAAnnularLayer.flagImageName)
            
            //For Pods bundle
            if flagImage == nil {
                let bundle = Bundle(for: LAAnnularLayer.self)
                if let url = bundle.url(forResource: "StepIndicator", withExtension: "bundle") {
                    let bundle = Bundle(url: url)
                    flagImage = UIImage(named: LAAnnularLayer.flagImageName, in: bundle, compatibleWith: nil)
                }
            }
            LAAnnularLayer.flagCGImage = flagImage?.cgImage
        }
    ...
    func updateStatus() {
        if isFinished {
            ...
            if self.displayNumber {
                ...
                if isCurrent {
                    self.drawCenterCircleAnimated(with: self.currentCircleColor?.cgColor)
                } else {
                    self.drawCenterCircleAnimated(with: self.nextCircleCenterColor?.cgColor, multiplier: 0.01)
                }
            }
        }
    }
    ...
    private func drawCenterCircleAnimated(with color: CGColor?, multiplier: CGFloat = 0.05) {
        let centerPath = UIBezierPath()
        let sideLength = fmin(self.frame.width, self.frame.height)
        let circlesRadius = sideLength / 2.0 - self.lineWidth - sideLength * multiplier
        ...
        self.centerCircleLayer.transform = LAAnnularLayer.originalScale
        self.centerCircleLayer.frame = self.bounds
        self.centerCircleLayer.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        self.centerCircleLayer.fillColor = color
    ...
    private func drawFullCircleAnimated() {
        ...
        self.fullCircleLayer.transform = LAAnnularLayer.originalScale
