...
@IBDesignable
public class LAStepIndicatorView: UIView {
    ...
    var annularLayers = [LAAnnularLayer]()
    private var horizontalLineLayers = [LALineLayer]()
    ...
    @IBInspectable public var currentStepCenterColor: UIColor = defaultTintColor {
        didSet {
            self.updateSubLayers()
        }
    }
    
    @IBInspectable public var nextStepCenterColor: UIColor = .clear {
        didSet {
            self.updateSubLayers()
        }
    }
    ...
    @IBInspectable var directionRaw: UInt {
        ...
        set {
            ...
            guard let direction = StepIndicatorViewDirection(rawValue: value) else { return }
    ...
    private func applyAnnularStyle(annularLayer: LAAnnularLayer) {
        annularLayer.annularDefaultColor = self.circleColor
        annularLayer.tintColor = self.circleTintColor
        annularLayer.currentCircleColor = self.currentStepCenterColor
        annularLayer.nextCircleCenterColor = self.nextStepCenterColor
    ...
    private func applyLineStyle(lineLayer: LALineLayer) {
